...
class App extends Component {
	...
	render() {
		...
		const freeUntilLabel = intl.formatMessage({
			id: "freeUntil"
		});

		const busyUntilLabel = intl.formatMessage({
			id: "busyUntil"
		});
		...
		if (!isAvailable) {
			...
			mainProps = { label: busyUntilLabel, time: busyUntil.format("LT") };
			state = "busy";
		} else if (nextEvent) {
			const freeUntil = moment(nextEvent.start);
			mainProps = { label: freeUntilLabel, time: freeUntil.format("LT") };
		} else {
			mainProps = { label: "", time: freeLabel };
		}
		...
		return (
			!isLoading && (
				<div className={cn("App", state)}>
					...
					<Main {...mainProps} intl={this.props.intl}>
			...
			slots && (
				<div className="slots-list">
					{slots.map(when => {
						...
						return <BookNow {...props} intl={this.props.intl} />;
					})}
				</div>
			)
		);
			...
			events.length > 0 && (
				<div className="event-list">
					{events.map(event => {
						...
						return <Event intl={this.props.intl} {...props} />;
					})}
				</div>
			)
		);
	...
	updateTime = () => {
		...
		const nextEvent =
			schedule.find(slot => !slot.available && now.isBefore(slot.start)) || null;
		...
		for (let i = 0; i < slots.length; i++) {
			if (slots[i].isAfter(nextEvent.start)) {
				if (i === 0 || slots[i - 1].isBefore(nextEvent.start)) {
					...
					// the previous slot equals the next event start
