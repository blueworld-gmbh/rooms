...
const normalizeEvent = function(apiEvent, now) {
	if (apiEvent.status !== "confirmed" || !apiEvent.start || !apiEvent.end) {
		return null;
	}
	...
	event.start = now
		.clone()
		.set({ hours: event.start.hours(), minutes: event.start.minutes() })
		.startOf("minute");
	event.end = now
		.clone()
		.set({ hours: event.end.hours(), minutes: event.end.minutes() })
		.startOf("minute");
...
const getNextEvent = function(events, now) {
	...
	for (let e of events) {
		if (now.isAfter(e.end)) {
			continue;
		}
		if (now.isBefore(e.start)) {
			return e;
		}
	}
	return null;
...
const getFreeSlots = function(events, now) {
	...
	for (let i = 0; i < events.length; i++) {
		...
		if (current.status !== "cancelled") {
			...
			let containingEvent =
				events
					.filter(e => current.end.isBetween(e.start, e.end))
...
const getNextFreeSlot = function(events, now) {
	...
	if (!freeSlot) {
		freeSlot = slots.find(s => now.isBefore(s.start)) || null;
	}
